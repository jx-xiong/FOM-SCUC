...
"""
    read_benchmark(name::AbstractString)::HPRUCInstance
...
# Example
```julia
instance = HPRUC.read_benchmark("matpower/case3375wp/2017-02-01")
```
"""
function read_benchmark(
    ...
    return HPRUC.read(filename, scale)
end

function _repair_scenario_names_and_probabilities!(
    scenarios::Vector{HPRUCScenario},
    path::Vector{String},
)::Nothing
...
"""
    read(path::AbstractString)::HPRUCInstance
...
```julia
instance = HPRUC.read("s1.json.gz")
```
"""
function read(path::String, scale::Bool)::HPRUCInstance
    scenarios = Vector{HPRUCScenario}()
    scenario = _read_scenario(path, scale)
    scenario.name = "s1"
    scenario.probability = 1.0
    scenarios = [scenario]
    instance =
        HPRUCInstance(time = scenario.time, scenarios = scenarios)
    return instance
end

"""
    read(path::Vector{String})::HPRUCInstance
...
```julia
instance = HPRUC.read(["s1.json.gz", "s2.json.gz"])
```
"""
function read(paths::Vector{String})::HPRUCInstance
    scenarios = HPRUCScenario[]
    for p in paths
        push!(scenarios, _read_scenario(p))
    end
    _repair_scenario_names_and_probabilities!(scenarios, paths)
    instance =
        HPRUCInstance(time = scenarios[1].time, scenarios = scenarios)
    return instance
end

function _read_scenario(path::String, scale::Bool)::HPRUCScenario
    if endswith(path, ".gz")
        scenario = _read(gzopen(path), scale)
    elseif endswith(path, ".json")
        scenario = _read(open(path), scale)
    else
...
function _read(file::IO, scale::Bool)::HPRUCScenario
    return _from_json(
...
function _from_json(json, use_scale; repair = true)::HPRUCScenario
    _migrate(json)
    ...
    if use_scale
        ...
        for (unit_name, dict) in json["Generators"]
            ...
            if lowercase(unit_type) === "thermal"
                ...
                curve_mw = dict["Production cost curve (MW)"]
                curve_cost = dict["Production cost curve (\$)"]
                max_power = max(max_power, maximum(curve_mw))
                max_cost = max(max_cost, maximum(curve_cost))
                # min_power = max(min_power, minimum(curve_mw))
                # min_cost = max(min_cost, minimum(curve_cost))

                # if maximum(curve_mw) > max_power
                #     max_power = maximum(curve_mw)
                #     max_cost = maximum(curve_cost)
                # end
            end
        end

        scale = max_power
        cost_scale = max_power / max_cost 
        # scale = min_power 
        # cost_scale = min_power / min_cost
    else
        cost_scale = 1.0
        scale = 1.0
        max_cost = 1.0
    ...
    @info "scale $(scale), cost_scale $(cost_scale)"

    println("Scale: $scale, cost_scale: $cost_scale")
    ...
    if "Reserves" in keys(json)
        for (reserve_name, dict) in json["Reserves"]
            r = Reserve(
                ...
                amount = timeseries(dict["Amount (MW)"]) ./ scale,
                thermal_units = [],
    ...
    for (unit_name, dict) in json["Generators"]
        ...
        if lowercase(unit_type) === "thermal"
            ...
            curve_mw = hcat(
                [
                    timeseries(dict["Production cost curve (MW)"][k]) ./ scale for
                    k in 1:K
                ]...,
            )
            curve_cost = hcat(
                [
                    timeseries(dict["Production cost curve (\$)"][k]) ./ (scale / cost_scale) for
                    k in 1:K
                ]...,
            )
            ...
            for k in 1:length(startup_delays)
                push!(
                    ...
                    StartupCategory(
                        ...
                        startup_costs[k] ./ (scale / cost_scale),
                    ),
                )
            end
            ...
            initial_power =
                scalar(dict["Initial power (MW)"], default = nothing) ./ scale
            initial_status =
            ...
            if initial_power === nothing
                ...
                if initial_status < 0 && initial_power > 1e-6
            ...
            unit = ThermalUnit(
                ...
                time_multiplier,
                scalar(dict["Ramp up limit (MW)"], default = 1e6) / scale,
                scalar(dict["Ramp down limit (MW)"], default = 1e6) / scale,
                scalar(dict["Startup limit (MW)"], default = 1e6) / scale,
                scalar(dict["Shutdown limit (MW)"], default = 1e6) / scale,
                initial_status,
                initial_power,
                startup_categories,
                unit_reserves,
        ...
        elseif lowercase(unit_type) === "profiled"
            ...
            pu = ProfiledUnit(
                unit_name,
                bus,
                timeseries(scalar(dict["Minimum power (MW)"], default = 0.0)) ./ scale,
                timeseries(dict["Maximum power (MW)"]) ./ scale,
                timeseries(dict["Cost (\$/MW)"]) .* cost_scale,
            )
    ...
    if "Price-sensitive loads" in keys(json)
        for (load_name, dict) in json["Price-sensitive loads"]
            ...
            load = PriceSensitiveLoad(
                load_name,
                bus,
                timeseries(dict["Demand (MW)"]) ./ scale,
                timeseries(dict["Revenue (\$/MW)"]) .* cost_scale,
            )
    ...
    if "Storage units" in keys(json)
        for (storage_name, dict) in json["Storage units"]
            bus = name_to_bus[dict["Bus"]]
            min_level =
                timeseries(scalar(dict["Minimum level (MWh)"], default = 0.0)) ./ scale
            max_level = timeseries(dict["Maximum level (MWh)"]) ./ scale
            storage = StorageUnit(
                storage_name,
                bus,
                min_level,
                ...
                timeseries(dict["Charge cost (\$/MW)"]) .* cost_scale,
                timeseries(dict["Discharge cost (\$/MW)"]) .* cost_scale,
                ...
                timeseries(dict["Maximum charge rate (MW)"]) ./ scale,
                timeseries(
                    scalar(dict["Minimum discharge rate (MW)"], default = 0.0),
                ) ./ scale,
                timeseries(dict["Maximum discharge rate (MW)"]) ./ scale,
                scalar(dict["Initial level (MWh)"], default = 0.0) ./ scale,
                scalar(
