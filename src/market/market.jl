...
function solve_market(
    ...
    instance_da = HPRUC.read(da_path)
    ...
    for rt_path in rt_paths
        ...
        instance_rt = HPRUC.read(rt_path)
        # check instance time 
        ...
        if !isempty(solution_rt) && !isempty(prev_initial_status)
            for g in sc.thermal_units
                ...
                g.initial_status = HPRUC._determine_initial_status(
...
function _build_and_optimize(
    ...
    # optimize model
    HPRUC.optimize!(model, settings.inner_method)
    solution = HPRUC.solution(model)
    # compute lmp and add to solution 
    if settings.lmp_method !== nothing
        lmp = HPRUC.compute_lmp(
            model,
            settings.lmp_method,
