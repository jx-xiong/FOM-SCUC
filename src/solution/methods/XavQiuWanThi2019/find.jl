...
function _find_violations(
    ...
    violations = HPRUC._find_violations(
        instance = instance,
        sc = sc,
...
`lodf` can be computed using HPRUC.injection_shift_factors and
HPRUC.line_outage_factors. The argument `overflow` specifies how much
