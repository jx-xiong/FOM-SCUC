...
instance = HPRUC.read("instance.json")

solution = HPRUC.optimize!(
    instance,
    TimeDecomposition(
...
function optimize!(
    ...
    for t_start in 1:method.time_increment:T
        ...
        sub_instance = HPRUC.slice(instance, t_start:t_end)
        # build and optimize the model 
        sub_model = HPRUC.build_model(
        ...
        HPRUC.optimize!(sub_model, method.inner_method)
        # get the result of each time period
        sub_solution = HPRUC.solution(sub_model)
